...
/* not declared static because a machine-specific function may call this one
   in some cases */
#undef FUNCNAME
#define FUNCNAME MPIR_Allgatherv_intra_MV2
#undef FCNAME
#define FCNAME MPIU_QUOTE(FUNCNAME)
int MPIR_Allgatherv_intra_MV2(void *sendbuf,
                              int sendcount,
                              ...
                              MPI_Datatype recvtype, MPID_Comm * comm_ptr, int *errflag)
{
    ...
    if ((total_count * recvtype_size < allgatherv_tuning_table[range].switchp) &&
#else
    if ((total_count * recvtype_size < MPIR_PARAM_ALLGATHER_LONG_MSG_SIZE) &&
        ...
        if (is_homogeneous) {
                ...
                if (dst < comm_size) {
                    ...
                    mpi_errno =
                        MPIC_Sendrecv_ft(((char *) tmp_buf +
                                          send_offset * recvtype_extent), curr_cnt,
            ...
            if (sendbuf != MPI_IN_PLACE) {
                ...
                /* if in_place specified, local data is found in recvbuf */
                mpi_errno =
                    MPIR_Pack_impl(((char *) recvbuf + displs[rank] * recvtype_extent),
                                   recvcounts[rank], recvtype, tmp_buf, tmp_buf_size,
                                   &position);
                if (mpi_errno)
                    MPIU_ERR_POP(mpi_errno);
            }
            ...
            while (mask < comm_size) {
                ...
                if (dst < comm_size) {
                    mpi_errno = MPIC_Sendrecv_ft(((char *) tmp_buf + send_offset),
                                                 ...
                                                 dst, MPIR_ALLGATHERV_TAG, comm, &status,
                                                 errflag);
                    if (mpi_errno) {
                        /* for communication errors, just record the error but continue */
                        *errflag = TRUE;
...
#if defined(_OSU_MVAPICH_) || defined(_OSU_PSM_)
    else if (total_count * recvtype_size < allgatherv_tuning_table[range].switchp)
#else
    else if (total_count * recvtype_size < MPIR_PARAM_ALLGATHER_LONG_MSG_SIZE)
#endif
    {
        ...
        if (sendbuf != MPI_IN_PLACE) {
            ...
            mpi_errno = MPIR_Localcopy(sendbuf, sendcount, sendtype,
                                       ((char *) recvbuf +
                                        displs[rank] * recvtype_extent), recvcounts[rank],
                                       recvtype);
            if (mpi_errno)
                MPIU_ERR_POP(mpi_errno);
        }
        ...
        while (tosend || torecv) {  /* While we have data to send or receive */
            ...
            if (!sendnow && !recvnow) {
                ...
                mpi_errno =
                    MPIC_Recv_ft(rbuf, recvnow, recvtype, left, MPIR_ALLGATHERV_TAG, comm,
                                 &status, errflag);
                if (mpi_errno) {
                    /* for communication errors, just record the error but continue */
                    *errflag = TRUE;
                ...
                mpi_errno =
                    MPIC_Send_ft(sbuf, sendnow, recvtype, right, MPIR_ALLGATHERV_TAG,
                                 comm, errflag);
                if (mpi_errno) {
                    /* for communication errors, just record the error but continue */
                    *errflag = TRUE;
                ...
                mpi_errno =
                    MPIC_Sendrecv_ft(sbuf, sendnow, recvtype, right, MPIR_ALLGATHERV_TAG,
...
#undef FCNAME
#define FCNAME MPIU_QUOTE(FUNCNAME)
int MPIR_Allgatherv_inter_MV2(void *sendbuf,
                              int sendcount,
                              MPI_Datatype sendtype,
                              void *recvbuf,
                              int *recvcounts,
...
*/
    ...
    mpi_errno =
        MPIR_Type_indexed_impl(remote_size, recvcounts, displs, recvtype, &newtype);
    if (mpi_errno)
        MPIU_ERR_POP(mpi_errno);
                        ...
                        MPI_Datatype recvtype, MPID_Comm * comm_ptr, int *errflag)
{
    ...
    if (comm_ptr->comm_kind == MPID_INTRACOMM) {
        /* intracommunicator */
        mpi_errno = MPIR_Allgatherv_intra_MV2(sendbuf, sendcount, sendtype,
        ...
        /* intercommunicator */
        mpi_errno = MPIR_Allgatherv_inter_MV2(sendbuf, sendcount, sendtype,
                                              recvbuf, recvcounts, displs, recvtype,
                                              comm_ptr, errflag);
        if (mpi_errno)
            MPIU_ERR_POP(mpi_errno);
